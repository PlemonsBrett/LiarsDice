...
protected:
  ...
  DiceTestFixture() { mock_rng = std::make_unique<MockRandomGenerator>(); }

  void create_dice_with_sequence(std::vector<int> sequence) {
    mock_rng->set_sequence(std::move(sequence));
    dice = std::make_unique<DiceImpl>(std::unique_ptr<IRandomGenerator>(mock_rng.release()));
  }

  void create_random_dice() {
    dice = std::make_unique<DiceImpl>(std::make_unique<MockRandomGenerator>());
  }
};
...
SCENARIO("Dice behavior follows game rules", "[dice][bdd]") {
  GIVEN("A newly created die") {
    ...
    AND_GIVEN("a specific random sequence") {
      auto rng2 = std::make_unique<MockRandomGenerator>(std::vector<int>{1, 6, 3, 4, 2, 5});
...
TEST_CASE("Dice performance benchmarks", "[dice][benchmark]") {
  ...
  BENCHMARK("Single roll performance") { return dice.roll(); };

  BENCHMARK("Face value getter performance") { return dice.get_face_value(); };

  BENCHMARK("Validation check performance") { return dice.is_valid_face_value(3); };
  ...
  BENCHMARK_ADVANCED("Bulk rolling performance")(Catch::Benchmark::Chronometer meter) {
    ...
    // Measure
    meter.measure([&dice_collection](int i) { dice_collection[i].roll(); });
  };
}
