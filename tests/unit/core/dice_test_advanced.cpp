...
public:
    ...
    double generate_normalized() override {
        return static_cast<double>(generate(0, 1000)) / 1000.0;
    }
...
public:
    bool match(unsigned int value) const {
        return value >= 1 && value <= 6;
    }
    
    std::string describe() const override {
        return "is a valid dice value (1-6)";
    }
};
...
TEST_CASE("Dice integration with controlled sequences", "[dice][integration]") {
    SECTION("Predictable sequence rollout") {
        ...
        for (size_t i = 0; i < sequence.size(); ++i) {
            if (i > 0) dice.roll(); // First value is set in constructor
