...
protected:
    ...
    void create_player_with_id(int id) {
        ...
        player = std::make_unique<PlayerImpl>(
            id,
...
TEST_CASE("Player properties", "[player][property]") {
    SECTION("Player ID validation") {
        ...
        if (player_id > 0) {
            ...
            REQUIRE_THROWS_AS(
                PlayerImpl(player_id, std::move(rng), std::move(dice_factory)),
...
TEST_CASE("Player performance benchmarks", "[player][benchmark]") {
    ...
    BENCHMARK("Roll all dice performance") {
        return fixture.player->roll_dice();
    };
    ...
    BENCHMARK("Get dice values performance") {
        return fixture.player->get_dice_values();
    };
}
...
TEST_CASE("Player with custom matchers", "[player][matchers]") {
    ...
    SECTION("Dice collection behavior") {
        ...
        for (const auto& die_ref : dice_refs) {
            REQUIRE_THAT(die_ref.get().get_face_value(), 
                        IsInRange(std::array{1u, 2u, 3u, 4u, 5u, 6u}));
        }
    }
}