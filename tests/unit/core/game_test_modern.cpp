...
public:
    [[nodiscard]] size_t get_current_player_index() const override {
    ...
    [[nodiscard]] bool is_game_active() const override {
        return game_active_;
    }
    
    void set_game_active(bool active) override {
        game_active_ = active;
    }
    
    [[nodiscard]] int get_round_number() const override {
        return round_number_;
    }
    
    void increment_round() override {
        ++round_number_;
    }
    
    [[nodiscard]] std::optional<Guess> get_last_guess() const override {
        return last_guess_;
    }
    
    void set_last_guess(const Guess& guess) override {
        last_guess_ = guess;
    }
    
    void clear_last_guess() override {
        last_guess_.reset();
    }
    
    IPlayer& get_player(size_t index) override {
    ...
    IPlayer& get_current_player() override {
    ...
    void setup_dependencies() override {
        ...
        container_->register_factory<ServiceFactory<IPlayer>>(
            []() -> std::unique_ptr<ServiceFactory<IPlayer>> {
                return std::make_unique<ServiceFactory<IPlayer>>(
                    []() -> std::unique_ptr<IPlayer> {
                        ...
                        return std::make_unique<PlayerImpl>(
                            player_id++, std::move(rng), std::move(dice_factory)
                        );
                    }
                );
            },
...
TEST_CASE("Game rule validation properties", "[game][property]") {
    ...
    SECTION("Sequential guess validation") {
        ...
        bool should_be_valid = (dice_count > 3) || 
                              (dice_count == 3 && face_value > 4);
...
TEST_CASE("Game exception safety", "[game][death-test]") {
    ...
    SECTION("Null dependency handling") {
        REQUIRE_THROWS_AS(
            GameImpl(nullptr, std::make_unique<MockRandomGenerator>(), nullptr),
    ...
    for (size_t i = 0; i < num_games; ++i) {
        ...
        while (!fixture.game_->is_game_over() && rounds < 100) {
            ...
            if (last_guess && fixture.main_rng_->generate_bool()) {
                ...
                // Make a guess
                unsigned int dice_count = last_guess 
                    ? last_guess->dice_count + 1 
                    : fixture.main_rng_->generate(1, 3);
