...
public:
  TestDiceFactory()
      : ServiceFactory<IDice>([]() -> std::unique_ptr<IDice> {
...
TEST_CASE("Player properties hold under various conditions", "[player][property]") {
  ...
  SECTION("Player ID validation") {
    ...
    if (player_id > 0) {
      ...
      REQUIRE_THROWS_AS(fixture.create_player(player_id), std::invalid_argument);
    }
  }
...
TEST_CASE("Player dice value counting and statistics", "[player][dice-counting]") {
  ...
  SECTION("Count dice with specific values") {
    ...
    for (unsigned int target_value = 1; target_value <= 6; ++target_value) {
      ...
      auto expected_count =
          static_cast<size_t>(std::count(dice_values.begin(), dice_values.end(), target_value));
...
TEST_CASE("Player performance benchmarks", "[player][benchmark]") {
  ...
  BENCHMARK("Count dice with value") { return player->count_dice_with_value(3); };

  BENCHMARK("Get all dice values") { return player->get_dice_values(); };

  BENCHMARK("Get dice references") { return player->get_dice(); };
}
...
TEST_CASE("Player exception safety", "[player][exceptions]") {
  ...
  SECTION("Constructor with null dependencies") {
    REQUIRE_THROWS_AS(PlayerImpl(1, nullptr, std::make_unique<TestDiceFactory>()),
                      std::invalid_argument);

    REQUIRE_THROWS_AS(PlayerImpl(1, std::make_unique<TestRandomGenerator>(), nullptr),
                      std::invalid_argument);
  }

  SECTION("Invalid player ID") {
    REQUIRE_THROWS_WITH(fixture.create_player(0),
                        Catch::Matchers::ContainsSubstring("Player ID must be positive"));

    REQUIRE_THROWS_WITH(fixture.create_player(-5),
                        Catch::Matchers::ContainsSubstring("Player ID must be positive"));
  }
}
...
TEST_CASE("Player with custom validation", "[player][matchers]") {
  ...
  SECTION("Player ID validation") {
    REQUIRE_THAT(player->get_id(),
                 Catch::Matchers::Predicate<int>([](int id) { return id > 0 && id < 1000; },
    ...
    for (auto value : dice_values) {
      REQUIRE_THAT(value,
                   Catch::Matchers::Predicate<unsigned int>(
                       [](unsigned int v) { return v >= 1 && v <= 6; }, "is valid dice value"));
    }
  }
}