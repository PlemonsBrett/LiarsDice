...
template <std::ranges::range Range> struct InRangeMatcher : Catch::Matchers::MatcherGenericBase {
...
template <std::ranges::range Range> auto IsInRange(Range &&range) {
...
template <typename T> class TestDataBuilder {
private:
...
public:
  ...
  template <typename... Args> TestDataBuilder &with(Args &&...args) {
    std::apply(
        [this](auto &&...a) { ((void)this->apply_property(std::forward<decltype(a)>(a)), ...); },
...
private:
  template <typename Prop> void apply_property(Prop &&prop) { prop(*object_); }
};
...
public:
  explicit MockRandomGenerator(std::vector<int> sequence = {}) : sequence_(std::move(sequence)) {}
