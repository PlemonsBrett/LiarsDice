...
public:
    ...
    [[nodiscard]] const IPlayer& get_player(size_t index) const override { return *players_.at(index); }
    IPlayer& get_current_player() override { return *players_[current_player_index_]; }
    [[nodiscard]] const IPlayer& get_current_player() const override { return *players_[current_player_index_]; }
...
TEST_CASE("Game rule validation properties", "[game][integration][property]") {
    ...
    SECTION("First guess validation") {
        ...
        bool should_be_valid = (dice_count >= 1 && dice_count <= 10 && 
                               face_value >= 1 && face_value <= 6);
...
TEST_CASE("DI container integration", "[game][integration][di]") {
    SECTION("Service resolution patterns") {
        ...
        container.register_service<IRandomGenerator, TestRandomGenerator>(
