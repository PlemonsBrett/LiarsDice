...
void list_players(DatabaseManager& db) {
  ...
  std::cout << std::setw(5) << "ID" << std::setw(20) << "Name" << std::setw(10) << "Games"
            << std::setw(10) << "Won" << std::setw(10) << "Lost" << std::setw(20) << "Created"
            << "\n";
  ...
  db.execute_prepared(stmt.value(), [&count](const PreparedStatement& row) {
    ...
    std::cout << std::setw(5) << id << std::setw(20) << name << std::setw(10) << total
              << std::setw(10) << won << std::setw(10) << lost << std::setw(20) << created << "\n";
    count++;
  });
...
void show_migration_history(SchemaManager& schema_mgr) {
  ...
  std::cout << std::setw(10) << "Version" << std::setw(30) << "Description" << std::setw(20)
            << "Applied At" << std::setw(15) << "Checksum" << "\n";
  std::cout << std::string(75, '-') << "\n";
  ...
  for (const auto& migration : history.value()) {
    ...
    std::cout << std::setw(10) << migration.version << std::setw(30) << migration.description
              << std::setw(20) << time_ss.str() << std::setw(15) << migration.checksum.substr(0, 12)
              << "...\n";
  }
}
...
void load_project_migrations(SchemaManager& schema_mgr) {
  ...
  if (!fs::exists(migrations_dir)) {
    ...
    schema_mgr.add_migration(std::make_unique<Migration>(1, "Create demo schema",
                                                         R"(
    ...
    schema_mgr.add_migration(std::make_unique<Migration>(2, "Add statistics table",
                                                         R"(
