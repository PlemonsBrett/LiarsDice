...
GameImpl::GameImpl(std::unique_ptr<interfaces::IGameState> game_state,
                   std::unique_ptr<interfaces::IRandomGenerator> random_generator,
                   std::unique_ptr<PlayerFactory> player_factory)
    : game_state_(std::move(game_state)), random_generator_(std::move(random_generator)),
...
bool GameImpl::is_game_over() const {
  ...
  auto active_count =
      std::ranges::count_if(players_, [](const auto &player) { return player->is_active(); });
...
std::string GameImpl::process_liar_call(int calling_player_id) {
  ...
  if (guess_was_correct) {
    ...
    if (calling_player_it != players_.end()) {
      ...
      result_message =
          std::format("Guess was correct! Player {} loses a die. Actual count: {}, Guessed: {}",
    ...
    if (guessing_player_it != players_.end()) {
      ...
      result_message =
          std::format("Guess was a lie! Player {} loses a die. Actual count: {}, Guessed: {}",
  ...
  std::erase_if(players_, [](const auto &player) { return !player->is_active(); });
...
interfaces::IGameState &GameImpl::get_game_state() { return *game_state_; }

const interfaces::IGameState &GameImpl::get_game_state() const { return *game_state_; }
...
size_t GameImpl::get_min_players() const { return kMinPlayers; }

size_t GameImpl::get_max_players() const { return kMaxPlayers; }

size_t GameImpl::get_initial_dice_per_player() const { return kInitialDicePerPlayer; }
...
int GameImpl::determine_winner() const {
  auto active_players =
      players_ | std::views::filter([](const auto &player) { return player->is_active(); });
...
std::string
GameImpl::validate_guess_rules(const interfaces::Guess &new_guess,
                               const std::optional<interfaces::Guess> &last_guess_opt) const {
  ...
  error_msg << std::format("Last guess was ({}, {})\n", last_guess.dice_count,
                           last_guess.face_value);
...
auto GameImpl::find_player_by_id(int player_id) -> decltype(players_.begin()) {
  return std::ranges::find_if(
      players_, [player_id](const auto &player) { return player->get_id() == player_id; });
}
