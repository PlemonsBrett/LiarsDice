...
bool PlayerImpl::has_dice() const {
    return !dice_.empty();
}
...
size_t PlayerImpl::count_dice_with_value(unsigned int face_value) const {
    ...
    auto count = std::ranges::count_if(dice_, 
        [face_value](const auto& die) {
            return die && die->get_face_value() == face_value;
        });
...
bool PlayerImpl::is_active() const {
    return has_dice();
}
...
std::unique_ptr<interfaces::IDice> PlayerImpl::create_die() {
    try {
        ...
        auto die = std::unique_ptr<interfaces::IDice>(
            static_cast<interfaces::IDice*>(raw_die)
        );
        
        // Initialize the die with a random roll
        if (die && random_generator_) {
            unsigned int initial_value = static_cast<unsigned int>(
                random_generator_->generate(
                    static_cast<int>(kMinDiceValue), 
                    static_cast<int>(kMaxDiceValue)
                )
            );
