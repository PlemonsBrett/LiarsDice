...
void DiceImpl::roll() {
    ...
    try {
        int rolled_value = random_generator_->generate(
            static_cast<int>(kMinFaceValue), 
        ...
        throw std::runtime_error(
            std::format("Failed to roll dice: {}", e.what())
        );
    }
}

unsigned int DiceImpl::get_face_value() const {
    return face_value_;
}

void DiceImpl::set_face_value(unsigned int value) {
...
std::unique_ptr<interfaces::IDice> DiceImpl::clone() const {
    ...
    throw std::runtime_error(
        "Clone operation not supported for DiceImpl. "
...
void DiceImpl::validate_and_set_face_value(unsigned int value) {
    if (!is_valid_face_value(value)) {
        throw std::invalid_argument(
            std::format("Invalid face value: {}. Must be between {} and {}", 
