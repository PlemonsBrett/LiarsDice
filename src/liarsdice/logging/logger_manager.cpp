...
void LoggerManager::initialize(const LoggerConfig& config) {
    ...
    try {
        ...
        if (!config_.config_file_path.empty() && 
            std::filesystem::exists(config_.config_file_path)) {
        ...
        if (logger_name == "default") {
            ...
            if (!spdlog_logger) {
                ...
                auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
                    config_.log_directory + "/" + logger_name + ".log", 
                    1024 * 1024 * 5, 3);
...
void LoggerManager::initialize_default_config() {
    ...
    auto file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
        config_.log_directory + "/liarsdice.log", 
        1024 * 1024 * 10,  // 10MB
        5);                // 5 files
...
void SpdlogLogger::log_structured(spdlog::level::level_enum level,
...
void SpdlogLogger::log_performance(const std::string& operation,
    ...
    spdlog_logger_->info("[PERF] {} {} ({}ms / {}μs)", 
                        message, ctx_str, duration_ms, duration_us);
#endif
}

void SpdlogLogger::set_correlation_id(CorrelationId id) {
    thread_correlation_id_ = id;
}

CorrelationId SpdlogLogger::get_correlation_id() const {
...
void SpdlogLogger::log_impl(spdlog::level::level_enum level,
...
void SpdlogLogger::log_with_context_impl(spdlog::level::level_enum level,
                                        const LogContext& context,
                                        const std::string& message,
                                        fmt::format_args args) const {
#ifdef LIARSDICE_ENABLE_LOGGING
...
void SpdlogLogger::log_source_impl(spdlog::level::level_enum level,
                                  const std::string& message,
    ...
    std::string source_info = fmt::format("{}:{}:{}", 
                                         location.file_name(), 
                                         location.line(), 
                                         location.function_name());
    
    if (thread_correlation_id_ != 0) {
        spdlog_logger_->log(level, "[CID:{}] [{}] {}", 
                           thread_correlation_id_, source_info, formatted_message);
    } else {
