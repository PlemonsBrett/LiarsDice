...
void JsonFileSource::stop_watching() {
    watching_ = false;
}
...
#ifdef LIARSDICE_HAS_NLOHMANN_JSON
void JsonFileSource::parse_json_recursive(const nlohmann::json& json_obj, const ConfigPath& base_path) {
...
void DefaultsSource::add_defaults(std::initializer_list<std::pair<ConfigPath, std::string>> defaults) {
    std::unique_lock lock(data_mutex_);
...
std::vector<std::pair<std::string, std::string>> EnvironmentWrapper::get_with_prefix(const std::string& prefix) {
...
std::vector<ArgumentParser::ParsedArgument> ArgumentParser::parse(std::span<const std::string_view> args) {
    std::vector<ParsedArgument> result;
    
    for (size_t i = 0; i < args.size(); ++i) {
        std::optional<std::string_view> next_arg = (i + 1 < args.size()) ? 
            std::optional<std::string_view>{args[i + 1]} : std::nullopt;
...
std::optional<ArgumentParser::ParsedArgument> ArgumentParser::parse_single(std::string_view arg, 
    ...
    if (is_long_option(arg)) {
        ...
        if (pos != std::string_view::npos) {
            ...
            return ParsedArgument{
                .key = normalize_key(arg.substr(0, pos)),
