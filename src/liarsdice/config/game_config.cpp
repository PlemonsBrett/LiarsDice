...
void GameConfig::load_game_rules() {
    rules.min_players = config_manager_.get_or<uint32_t>(ConfigPath{"game.rules.min_players"}, rules.min_players);
    rules.max_players = config_manager_.get_or<uint32_t>(ConfigPath{"game.rules.max_players"}, rules.max_players);
    rules.dice_per_player = config_manager_.get_or<uint32_t>(ConfigPath{"game.rules.dice_per_player"}, rules.dice_per_player);
    ...
    rules.allow_spectators = config_manager_.get_or<bool>(ConfigPath{"game.rules.allow_spectators"}, rules.allow_spectators);
    ...
    rules.show_dice_count = config_manager_.get_or<bool>(ConfigPath{"game.rules.show_dice_count"}, rules.show_dice_count);
    rules.enable_undo = config_manager_.get_or<bool>(ConfigPath{"game.rules.enable_undo"}, rules.enable_undo);
}
...
void GameConfig::load_ui_preferences() {
    ...
    ui.show_animations = config_manager_.get_or<bool>(ConfigPath{"ui.show_animations"}, ui.show_animations);
    ui.show_tooltips = config_manager_.get_or<bool>(ConfigPath{"ui.show_tooltips"}, ui.show_tooltips);
    ui.confirm_actions = config_manager_.get_or<bool>(ConfigPath{"ui.confirm_actions"}, ui.confirm_actions);
    ui.animation_speed = config_manager_.get_or<uint32_t>(ConfigPath{"ui.animation_speed"}, ui.animation_speed);
    ui.language = config_manager_.get_or<std::string>(ConfigPath{"ui.language"}, ui.language);
    ui.accessibility_mode = config_manager_.get_or<bool>(ConfigPath{"ui.accessibility_mode"}, ui.accessibility_mode);
...
void GameConfig::load_sound_config() {
    ...
    sound.master_volume = config_manager_.get_or<uint32_t>(ConfigPath{"sound.master_volume"}, sound.master_volume);
    sound.effects_volume = config_manager_.get_or<uint32_t>(ConfigPath{"sound.effects_volume"}, sound.effects_volume);
    sound.ambient_volume = config_manager_.get_or<uint32_t>(ConfigPath{"sound.ambient_volume"}, sound.ambient_volume);
    sound.mute_when_unfocused = config_manager_.get_or<bool>(ConfigPath{"sound.mute_when_unfocused"}, sound.mute_when_unfocused);
}
...
void GameConfig::load_ai_config() {
    ...
    ai.enable_ai_learning = config_manager_.get_or<bool>(ConfigPath{"ai.enable_learning"}, ai.enable_ai_learning);
    ai.show_ai_thinking = config_manager_.get_or<bool>(ConfigPath{"ai.show_thinking"}, ai.show_ai_thinking);
    ...
    ai.bluff_frequency = config_manager_.get_or<double>(ConfigPath{"ai.bluff_frequency"}, ai.bluff_frequency);
    ai.conservative_factor = config_manager_.get_or<double>(ConfigPath{"ai.conservative_factor"}, ai.conservative_factor);
}

void GameConfig::load_network_config() {
    network.default_port = config_manager_.get_or<uint32_t>(ConfigPath{"network.default_port"}, network.default_port);
    ...
    network.max_reconnect_attempts = config_manager_.get_or<uint32_t>(ConfigPath{"network.max_reconnect_attempts"}, network.max_reconnect_attempts);
    network.enable_lan_discovery = config_manager_.get_or<bool>(ConfigPath{"network.enable_lan_discovery"}, network.enable_lan_discovery);
    network.server_region = config_manager_.get_or<std::string>(ConfigPath{"network.server_region"}, network.server_region);
}
...
void GameConfig::save_game_rules() {
    ...
    config_manager_.set<std::string>(ConfigPath{"game.rules.dice_faces"}, to_string(rules.dice_faces));
    ...
    config_manager_.set<std::string>(ConfigPath{"game.rules.timeout_action"}, to_string(rules.timeout_action));
...
void GameConfig::save_ai_config() {
    config_manager_.set<std::string>(ConfigPath{"ai.default_difficulty"}, to_string(ai.default_difficulty));
...
void GameConfig::save_network_config() {
    ...
    config_manager_.set<int64_t>(ConfigPath{"network.connection_timeout"}, network.connection_timeout.count());
    config_manager_.set<uint32_t>(ConfigPath{"network.max_reconnect_attempts"}, network.max_reconnect_attempts);
    config_manager_.set<bool>(ConfigPath{"network.enable_lan_discovery"}, network.enable_lan_discovery);
...
std::string to_string(DiceFaces faces) {
    return std::to_string(static_cast<uint8_t>(faces));
}

std::string to_string(GameVariant variant) {
    switch (variant) {
        case GameVariant::Classic: return "classic";
        case GameVariant::Perudo: return "perudo";
        case GameVariant::Dudo: return "dudo";
        case GameVariant::Challenge: return "challenge";
...
std::string to_string(TimeoutAction action) {
    switch (action) {
        case TimeoutAction::Forfeit: return "forfeit";
        case TimeoutAction::AutoCall: return "auto_call";
        case TimeoutAction::RandomBid: return "random_bid";
        case TimeoutAction::Extend: return "extend";
...
std::string to_string(UITheme theme) {
    switch (theme) {
        case UITheme::Auto: return "auto";
        case UITheme::Light: return "light";
        case UITheme::Dark: return "dark";
        case UITheme::HighContrast: return "high_contrast";
        default: return "auto";
    }
}
...
std::string to_string(SoundMode mode) {
    switch (mode) {
        ...
        case SoundMode::Essential: return "essential";
        case SoundMode::Full: return "full";
        default: return "full";
    }
}
...
std::optional<Difficulty> parse_difficulty(const std::string& str) {
    if (str == "beginner") return Difficulty::Beginner;
...
std::optional<DiceFaces> parse_dice_faces(const std::string& str) {
    if (str == "4") return DiceFaces::Four;
    if (str == "6") return DiceFaces::Six;
    if (str == "8") return DiceFaces::Eight;
    if (str == "10") return DiceFaces::Ten;
    if (str == "12") return DiceFaces::Twelve;
    if (str == "20") return DiceFaces::Twenty;
    return std::nullopt;
}

std::optional<GameVariant> parse_game_variant(const std::string& str) {
    if (str == "classic") return GameVariant::Classic;
    if (str == "perudo") return GameVariant::Perudo;
    if (str == "dudo") return GameVariant::Dudo;
    if (str == "challenge") return GameVariant::Challenge;
    return std::nullopt;
}

std::optional<TimeoutAction> parse_timeout_action(const std::string& str) {
    if (str == "forfeit") return TimeoutAction::Forfeit;
    if (str == "auto_call") return TimeoutAction::AutoCall;
    if (str == "random_bid") return TimeoutAction::RandomBid;
    if (str == "extend") return TimeoutAction::Extend;
    return std::nullopt;
}

std::optional<UITheme> parse_ui_theme(const std::string& str) {
    if (str == "auto") return UITheme::Auto;
    if (str == "light") return UITheme::Light;
    if (str == "dark") return UITheme::Dark;
    if (str == "high_contrast") return UITheme::HighContrast;
    return std::nullopt;
}

std::optional<SoundMode> parse_sound_mode(const std::string& str) {
    if (str == "off") return SoundMode::Off;
    if (str == "essential") return SoundMode::Essential;
    if (str == "full") return SoundMode::Full;
    return std::nullopt;
}
