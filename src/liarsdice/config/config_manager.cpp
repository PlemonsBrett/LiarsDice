...
void ConfigManager::add_source(std::unique_ptr<IConfigSource> source) {
  ...
  auto insert_pos =
      std::ranges::upper_bound(sources_, source->get_priority(), [](int priority, const auto &src) {
        return priority > src->get_priority();
      });
...
void ConfigManager::remove_source(const std::string &name) {
  ...
  auto it =
      std::ranges::find_if(sources_, [&name](const auto &src) { return src->get_name() == name; });
...
ConfigManager::get_section(const ConfigPath &section_path) const {
  std::shared_lock lock(mutex_);
...
std::vector<std::string> ConfigManager::validate() const {
  ...
  for (const auto &source : sources_) {
    try {
      ...
      errors.push_back(
          std::format("Source '{}' validation failed: {}", source->get_name(), e.what()));
    }
  }
...
void ConfigManager::notify_change(const ConfigPath &path, const ConfigValue &old_value,
                                  const ConfigValue &new_value) {
  ConfigChangeEvent event{.path = path,
                          .old_value = old_value,
                          .new_value = new_value,
