...
private:
  ...
  std::shared_ptr<spdlog::sinks::sink> create_sink_from_config(const std::string &sink_type,
...
public:
  ...
  void log_structured(spdlog::level::level_enum level, const LogContext &context,
                      const std::string &event_type, const std::string &message) const override;

  void log_performance(const std::string &operation, std::chrono::nanoseconds duration,
...
protected:
  void log_impl(spdlog::level::level_enum level, const std::string &message,
                fmt::format_args args) const override;

  void log_with_context_impl(spdlog::level::level_enum level, const LogContext &context,
                             const std::string &message, fmt::format_args args) const override;

  void log_source_impl(spdlog::level::level_enum level, const std::string &message,
                       const std::source_location &location, fmt::format_args args) const override;
...
public:
  ...
  ~CorrelationScope() { logger_.set_correlation_id(previous_id_); }
...
#define GET_COMPONENT_LOGGER(component)                                                            \
  liarsdice::logging::LoggerManager::instance().get_component_logger(component)
#define LOG_CORRELATION_SCOPE(logger, id)                                                          \
  liarsdice::logging::CorrelationScope scope_##__LINE__(logger, id)
#define GENERATE_CORRELATION_ID()                                                                  \
  liarsdice::logging::LoggerManager::instance().generate_correlation_id()
#else
