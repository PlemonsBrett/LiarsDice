...
namespace spdlog {
enum class level : int { trace = 0, debug = 1, info = 2, warn = 3, err = 4, critical = 5, off = 6 };
}
#endif
...
struct LogContext {
  ...
  LogContext() = default;
  LogContext(std::string_view comp, std::string_view op = "") : component(comp), operation(op) {}
};
...
public:
  ...
  void log(spdlog::level::level_enum level, const std::string &message, Args &&...args) const {
   ...
   */
  template <typename... Args>
  void log_with_context(spdlog::level::level_enum level, const LogContext &context,
                        const std::string &message, Args &&...args) const {
  ...
  template <typename... Args> void warn(const std::string &message, Args &&...args) const {
  ...
  template <typename... Args> void critical(const std::string &message, Args &&...args) const {
   ...
   */
  template <typename... Args>
  void log_source(spdlog::level::level_enum level, const std::string &message,
   ...
   */
  virtual void log_structured(spdlog::level::level_enum level, const LogContext &context,
                              const std::string &event_type, const std::string &message) const = 0;

  /**
   * @brief Performance logging with timing information
   */
  virtual void log_performance(const std::string &operation, std::chrono::nanoseconds duration,
...
protected:
  virtual void log_impl(spdlog::level::level_enum level, const std::string &message,
                        fmt::format_args args) const = 0;

  virtual void log_with_context_impl(spdlog::level::level_enum level, const LogContext &context,
                                     const std::string &message, fmt::format_args args) const = 0;

  virtual void log_source_impl(spdlog::level::level_enum level, const std::string &message,
...
private:
  ...
  template <typename... Args>
  void log_impl(spdlog::level::level_enum level, const std::string &message, Args &&...args) const {
  ...
  template <typename... Args>
  void log_with_context_impl(spdlog::level::level_enum level, const LogContext &context,
                             const std::string &message, Args &&...args) const {
    if constexpr (sizeof...(args) > 0) {
  ...
  template <typename... Args>
  void log_source_impl(spdlog::level::level_enum level, const std::string &message,
...
public:
  ScopedTimer(const ILogger &logger, std::string operation, LogContext context = {})
      : logger_(logger), operation_(std::move(operation)), context_(std::move(context)),
...
#define LOG_TRACE(logger, msg, ...)                                                                \
  (logger).log_source(spdlog::level::trace, msg,                                                   \
                      std::source_location::current() __VA_OPT__(, ) __VA_ARGS__)
#define LOG_DEBUG(logger, msg, ...)                                                                \
  (logger).log_source(spdlog::level::debug, msg,                                                   \
                      std::source_location::current() __VA_OPT__(, ) __VA_ARGS__)
#define LOG_INFO(logger, msg, ...)                                                                 \
  (logger).log_source(spdlog::level::info, msg,                                                    \
                      std::source_location::current() __VA_OPT__(, ) __VA_ARGS__)
#define LOG_WARN(logger, msg, ...)                                                                 \
  (logger).log_source(spdlog::level::warn, msg,                                                    \
                      std::source_location::current() __VA_OPT__(, ) __VA_ARGS__)
#define LOG_ERROR(logger, msg, ...)                                                                \
  (logger).log_source(spdlog::level::err, msg,                                                     \
                      std::source_location::current() __VA_OPT__(, ) __VA_ARGS__)
#define LOG_CRITICAL(logger, msg, ...)                                                             \
  (logger).log_source(spdlog::level::critical, msg,                                                \
                      std::source_location::current() __VA_OPT__(, ) __VA_ARGS__)
