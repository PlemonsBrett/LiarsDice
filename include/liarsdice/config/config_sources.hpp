...
public:
   ...
   */
  template <typename T> void add_default(const ConfigPath &path, const T &value) {
...
public:
  ...
  get_with_prefix(const std::string &prefix);
};
...
public:
   ...
   */
  static std::optional<ParsedArgument>
  parse_single(std::string_view arg, std::optional<std::string_view> next_arg = std::nullopt);
