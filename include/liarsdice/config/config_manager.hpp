...
public:
   ...
   */
  template <typename T> std::optional<T> get(const ConfigPath &path) const {
   ...
   */
  template <typename T> T get_or(const ConfigPath &path, const T &default_value) const {
   ...
   */
  template <typename T> T get_required(const ConfigPath &path) const {
    if (auto value = get<T>(path)) {
      return *value;
    }
   ...
   */
  template <typename T> void set(const ConfigPath &path, const T &value) {
...
private:
  template <typename T> std::optional<T> get_internal(const ConfigPath &path) const {
    ...
    for (const auto &source : sources_) {
      if (source->has_value(path)) {
        if (auto raw_value = source->get_raw_value(path)) {
          ...
          if (auto config_value = ConfigValue::from_string(*raw_value, get_type_index<T>())) {
  ...
  template <typename T> static constexpr std::size_t get_type_index() {
  ...
  void notify_change(const ConfigPath &path, const ConfigValue &old_value,
 ...
 */
template <typename T> std::optional<T> get_config(const ConfigPath &path) {
  return global_config().get<T>(path);
}

template <typename T> T get_config_or(const ConfigPath &path, const T &default_value) {
  return global_config().get_or<T>(path, default_value);
}

template <typename T> T get_required_config(const ConfigPath &path) {
  return global_config().get_required<T>(path);
}

template <typename T> void set_config(const ConfigPath &path, const T &value) {
