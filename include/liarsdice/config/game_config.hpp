 ...
 */
constexpr auto percentage_validator() {
    return make_range_validator<uint32_t>(0, 100);
}

/**
 * @brief Validate extended percentage value (50-200)
 */
constexpr auto extended_percentage_validator() {
    return make_range_validator<uint32_t>(50, 200);
}

/**
 * @brief Validate port number
 */
constexpr auto port_validator() {
    return make_range_validator<uint16_t>(1024, 65535);
}
