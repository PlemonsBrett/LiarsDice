...
public:
  explicit ConfigException(const std::string &message,
      ...
      : std::runtime_error(std::format("Config error at {}:{}: {}", location.file_name(),
 ...
 */
using ConfigVariant = std::variant<std::monostate, // Represents null/unset value
                                   bool, int32_t, int64_t, uint32_t, uint64_t, double, std::string,
...
public:
  ...
  constexpr ConfigValue(T &&value) noexcept : value_(std::forward<T>(value)), is_set_(true) {}
  ...
  constexpr ConfigValue(T &&value, T &&default_value) noexcept
      : value_(std::forward<T>(value)), default_value_(std::forward<T>(default_value)),
        is_set_(true) {}

  /**
   * @brief Get the value with type checking
   */
  template <typename T> constexpr std::optional<T> get() const noexcept {
    if (!is_set_) {
      return std::nullopt;
    }
   ...
   */
  template <typename T> constexpr T get_or(const T &default_val) const noexcept {
    if (auto val = get<T>()) {
      return *val;
    }
    return default_val;
   ...
   */
  template <typename T> constexpr T get_required() const {
    if (auto val = get<T>()) {
      return *val;
    }
    throw ConfigException(
        std::format("Required configuration value of type '{}' not found", typeid(T).name()));
  }
   ...
   */
  constexpr std::size_t type_index() const noexcept { return value_.index(); }

  /**
   * @brief Set new value
   */
  template <ConfigValueType T> constexpr void set(T &&new_value) noexcept {
   ...
   */
  static std::optional<ConfigValue> from_string(const std::string &str,
 ...
 */
template <typename T> class ConfigValidator {
public:
   ...
   */
  std::string get_error_message() const { return last_valid_ ? "" : error_message_(); }
...
constexpr auto make_range_validator(T min_val, T max_val) {
  return ConfigValidator<T>(
      ...
      [min_val, max_val]() {
        return std::format("Value must be between {} and {}", min_val, max_val);
      });
}
