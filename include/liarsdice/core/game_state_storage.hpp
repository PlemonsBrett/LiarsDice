...
public:
    ...
    template<typename U>
    GameStateAllocator(const GameStateAllocator<U>&) noexcept {}
...
template<typename T>
boost::pool<> GameStateAllocator<T>::pool_(sizeof(T));

template<typename T>
typename GameStateAllocator<T>::pointer 
GameStateAllocator<T>::allocate(size_type n) {
    if (n == 1) {
...
template<typename T>
void GameStateAllocator<T>::deallocate(pointer p, size_type n) noexcept {
    if (n == 1) {
        pool_.free(p);
    } else {
        ::operator delete(p);
