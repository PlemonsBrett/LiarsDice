...
namespace liarsdice::statistics {
  ...
  public:
    using value_type = T;
    using axis_type = typename std::conditional_t<std::is_integral_v<T>, bh::axis::integer<T>,
                                                  bh::axis::regular<T> >;
    ...
    Histogram(std::size_t bins, T min, T max) {
      if constexpr (std::is_integral_v<T>) {
        hist_ = bh::make_histogram(bh::axis::integer<T>(min, max, "value"));
      } else {
        hist_ = bh::make_histogram(bh::axis::regular<T>(bins, min, max, "value"));
      }
    }
     ...
     */
    [[nodiscard]] double total_count() const { return total_count_; }

    /**
     * @brief Get number of bins
     * @return Number of histogram bins
     */
    [[nodiscard]] std::size_t bin_count() const { return hist_.axis().size(); }
     ...
     */
    [[nodiscard]] double standard_deviation() const { return std::sqrt(variance()); }
     ...
     */
    [[nodiscard]] const histogram_type& get_histogram() const { return hist_; }
  ...
  public:
    using histogram_type
        = decltype(bh::make_histogram(std::declval<typename Histogram<T1>::axis_type>(),
    ...
    Histogram2D(std::size_t bins_x, T1 min_x, T1 max_x, std::size_t bins_y, T2 min_y, T2 max_y) {
      if constexpr (std::is_integral_v<T1> && std::is_integral_v<T2>) {
        hist_ = bh::make_histogram(bh::axis::integer<T1>(min_x, max_x, "x"),
        ...
        hist_ = bh::make_histogram(bh::axis::regular<T1>(bins_x, min_x, max_x, "x"),
                                   bh::axis::integer<T2>(min_y, max_y, "y"));
      } else {
        hist_ = bh::make_histogram(bh::axis::regular<T1>(bins_x, min_x, max_x, "x"),
                                   bh::axis::regular<T2>(bins_y, min_y, max_y, "y"));
    ...
    void add(T1 x, T2 y, double weight = 1.0) { hist_(x, y, bh::weight(weight)); }
