...
struct TimePoint {
    ...
    TimePoint() = default;
    TimePoint(T val) 
        : timestamp(std::chrono::steady_clock::now()), value(std::move(val)) {}
...
public:
    ...
    [[nodiscard]] std::vector<double> exponential_moving_average(double alpha) const {
        ...
        for (std::size_t i = 1; i < buffer_.size(); ++i) {
            double new_ema = alpha * static_cast<double>(buffer_[i].value) 
                           + (1 - alpha) * ema.back();
     ...
     */
    [[nodiscard]] std::vector<double> savitzky_golay_filter(
        std::size_t window_size, std::size_t poly_order) const {
        if (window_size % 2 == 0 || window_size > buffer_.size() || 
