...
namespace liarsdice::statistics {
  ...
  public:
     ...
     */
    [[nodiscard]] virtual double standard_deviation() const { return std::sqrt(variance()); }
   ...
   */
  template <typename Distribution> class DistributionWrapper : public IProbabilityDistribution {
  public:
    using distribution_type = Distribution;

    template <typename... Args> explicit DistributionWrapper(Args&&... args)
        : dist_(std::forward<Args>(args)...) {}

    [[nodiscard]] double pdf(double x) const override { return bm::pdf(dist_, x); }

    [[nodiscard]] double cdf(double x) const override { return bm::cdf(dist_, x); }

    [[nodiscard]] double quantile(double p) const override { return bm::quantile(dist_, p); }

    [[nodiscard]] double mean() const override { return bm::mean(dist_); }

    [[nodiscard]] double variance() const override { return bm::variance(dist_); }
  ...
  public:
    NormalDistribution(double mean = 0.0, double std_dev = 1.0)
        : DistributionWrapper(mean, std_dev), sampler_(mean, std_dev) {}

    double sample(br::mt19937& gen) const override { return sampler_(gen); }
  ...
  public:
    BinomialDistribution(unsigned int n, double p)
        : DistributionWrapper(static_cast<double>(n), p), n_(n), p_(p), sampler_(n, p) {}

    double sample(br::mt19937& gen) const override { return static_cast<double>(sampler_(gen)); }
  ...
  public:
    explicit PoissonDistribution(double lambda)
        : DistributionWrapper(lambda), lambda_(lambda), sampler_(lambda) {}
  ...
  public:
    UniformDistribution(double a = 0.0, double b = 1.0)
        : DistributionWrapper(a, b), a_(a), b_(b), sampler_(a, b) {}
  ...
  public:
    BetaDistribution(double alpha, double beta)
        : DistributionWrapper(alpha, beta), alpha_(alpha), beta_(beta) {
  ...
  public:
     ...
     */
    static std::unique_ptr<IProbabilityDistribution> create_normal(double mean = 0.0,
                                                                   double std_dev = 1.0) {
     ...
     */
    static std::unique_ptr<IProbabilityDistribution> create_binomial(unsigned int n, double p) {
     ...
     */
    static std::unique_ptr<IProbabilityDistribution> create_uniform(double a = 0.0,
                                                                    double b = 1.0) {
     ...
     */
    static std::unique_ptr<IProbabilityDistribution> create_exponential(double lambda = 1.0) {
     ...
     */
    static std::unique_ptr<IProbabilityDistribution> create_beta(double alpha, double beta) {
  ...
  public:
     ...
     */
    static std::pair<double, double> kolmogorov_smirnov_test(const std::vector<double>& data,
     ...
     */
    static std::pair<double, int> chi_square_test(const std::vector<double>& observed,
  ...
  public:
     ...
     */
    static std::unique_ptr<BetaDistribution> update_beta(const BetaDistribution& prior,
     ...
     */
    static std::pair<double, double> credible_interval(const BetaDistribution& dist,
