...
class ServiceFactory : public interfaces::IServiceFactory {
private:
...
public:
   ...
   */
  ServiceFactory() : factory_func_([]() { return std::make_unique<TImplementation>(); }) {}
   ...
   */
  std::unique_ptr<TInterface> create() override { return factory_func_(); }
...
class DependentServiceFactory : public interfaces::IServiceFactory {
public:
  /**
...
private:
  ...
  create_with_resolved_deps(std::tuple<ServiceContainer::ServiceResult<TDeps>...> deps) {
    // Check if all dependencies were resolved successfully
    auto check_deps = [](const auto &...dep_results) { return (dep_results.has_value() && ...); };
 ...
 */
template <interfaces::ServiceInterface TInterface> class ServiceRegistration {
private:
...
public:
  explicit ServiceRegistration(ServiceContainer &container) : container_(&container) {}
  ...
  void use(Args &&...args) {
    container_->register_service<TInterface, TImplementation>(lifetime_, name_,
...
public:
   ...
   */
  std::unique_ptr<ServiceContainer> build() { return std::move(container_); }

  /**
   * @brief Get reference to container for direct access
   */
  ServiceContainer &get_container() { return *container_; }
};
