...
namespace liarsdice::di {
  ...
  template <typename T, typename... Args> auto make_factory() {
    return [](Args... args) { return std::make_shared<T>(args...); };
  }

  // Binding helpers
  template <typename Interface, typename Implementation> auto bind_interface() {
    return di::bind<Interface>().to<Implementation>();
  }

  template <typename Interface, typename Implementation> auto bind_singleton() {
    return di::bind<Interface>().to<Implementation>().in(singleton);
  }

  template <typename T> auto bind_instance(std::shared_ptr<T> instance) {
    return di::bind<T>().to(instance);
  }

  // Configuration helper
  template <typename... Bindings> auto make_injector(Bindings&&... bindings) {
  ...
  namespace testing {
    ...
    template <typename Interface, typename Mock> auto bind_mock() {
      return di::bind<Interface>().to<Mock>().in(singleton);
    }

    // Test injector with mocks
    template <typename... MockBindings> auto make_test_injector(MockBindings&&... bindings) {
...
#define DI_BIND_INSTANCE(type, instance) liarsdice::di::bind_instance<type>(instance)

#define DI_MAKE_INJECTOR(...) liarsdice::di::make_injector(__VA_ARGS__)

#define DI_CREATE(injector, type) injector.create<std::shared_ptr<type>>()

#define DI_TEST_BIND(interface, mock) liarsdice::di::testing::bind_mock<interface, mock>()