...
namespace liarsdice::di {
  ...
  public:
    ...
    void register_service(ServiceLifetime lifetime = ServiceLifetime::Transient) {
      ...
      auto factory
          = []() -> std::shared_ptr<TInterface> { return std::make_shared<TImplementation>(); };
    ...
    template <typename T> std::shared_ptr<T> resolve() {
      ...
      if (it == services_.end()) {
        throw ServiceContainerException("Service not registered: " + std::string(typeid(T).name()));
      }
...
#define RESOLVE_SERVICE(container, type) container.resolve<type>()
