...
public:
  ...
  void register_service(const std::string &name = "",
    ...
    auto factory = [args = std::tuple<Args...>(std::forward<Args>(args)...)]() -> void * {
      return std::apply(
          [](auto &&...args) { return new TImplementation(std::forward<decltype(args)>(args)...); },
          args);
    };
  ...
  void register_factory(std::function<std::unique_ptr<TInterface>()> factory,
    ...
    auto deleter = [](void *ptr) { std::unique_ptr<TInterface>(static_cast<TInterface *>(ptr)); };
   ...
   */
  template <typename T> std::unique_ptr<T> resolve(const std::string &name) {
