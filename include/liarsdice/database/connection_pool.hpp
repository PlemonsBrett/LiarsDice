...
namespace liarsdice::database {
  ...
  public:
    ...
    public:
      ...
      PooledConnection(PooledConnection&& other) noexcept
          : connection_(std::move(other.connection_)), returner_(std::move(other.returner_)) {
     ...
     */
    ConnectionPool(boost::asio::io_context& io_context, const std::string& connection_string,
    ...
    PooledConnection acquire(std::chrono::milliseconds timeout = std::chrono::milliseconds(5000)) {
      ...
      return PooledConnection(conn, [this](ConnectionPtr c) { return_connection(c); });
    }
    ...
    PoolStats get_stats() const {
      ...
      return {total_connections_, available_connections_.size(),
              total_connections_ - available_connections_.size(), failed_connections_};
    }
  ...
  private:
     ...
     */
    size_t get_active_count() const { return total_connections_ - available_connections_.size(); }
