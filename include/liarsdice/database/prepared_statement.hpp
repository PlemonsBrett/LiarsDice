...
public:
    ...
    bool bind(int index, const ValueType& value) {
        ...
        return std::visit([this, index](auto&& arg) -> bool {
            ...
            if constexpr (std::is_same_v<T, std::monostate>) {
                ...
                return sqlite3_bind_text(stmt_.get(), index, arg.c_str(), 
                                       static_cast<int>(arg.size()), SQLITE_TRANSIENT) == SQLITE_OK;
            } else if constexpr (std::is_same_v<T, std::vector<uint8_t>>) {
                return sqlite3_bind_blob(stmt_.get(), index, arg.data(), 
                                       static_cast<int>(arg.size()), SQLITE_TRANSIENT) == SQLITE_OK;
            }
            return false;
        }, value);
     ...
     */
    bool reset() {
        return stmt_ && sqlite3_reset(stmt_.get()) == SQLITE_OK;
    }
     ...
     */
    bool clear_bindings() {
        return stmt_ && sqlite3_clear_bindings(stmt_.get()) == SQLITE_OK;
    }
     ...
     */
    int step() {
        return stmt_ ? sqlite3_step(stmt_.get()) : SQLITE_MISUSE;
    }
