  ...
  public:
    ...
    DatabaseResult<void> execute(const std::string& sql) {
      ...
      try {
        ...
        if (error) {
          BOOST_LOG_TRIVIAL(error) << "Query failed: " << sql << " Error: " << error.message();
          return DatabaseError(DatabaseErrorType::QueryFailed, "Query execution failed", error);
        }

        BOOST_LOG_TRIVIAL(trace) << "Query executed in " << duration.count() << "μs: " << sql;
    ...
    DatabaseResult<std::shared_ptr<PreparedStatement>> prepare(const std::string& sql) {
      ...
      if (stats.cache_memory_estimate > MAX_CACHE_MEMORY_MB * 1024 * 1024) {
        ...
        BOOST_LOG_TRIVIAL(warning)
            << "Cache memory limit exceeded, cleared " << to_remove << " statements";
      }
      ...
      try {
        ...
        if (result != SQLITE_OK) {
          ...
          BOOST_LOG_TRIVIAL(error)
              << "Failed to prepare statement: " << sql << " Error: " << error_msg;
    ...
    DatabaseResult<int> execute_prepared(std::shared_ptr<PreparedStatement> stmt,
      ...
      if (!stmt || !*stmt) {
        return DatabaseError(DatabaseErrorType::InvalidParameter, "Invalid prepared statement");
      }
      ...
      try {
        ...
        BOOST_LOG_TRIVIAL(trace) << "Prepared statement executed in " << duration.count() << "μs, "
    ...
    DatabaseResult<void> begin_transaction() {
      ...
      if (error) {
        ...
        return DatabaseError(DatabaseErrorType::TransactionFailed, "Failed to begin transaction",
                             error);
      }
    ...
    DatabaseResult<void> commit_transaction() {
      if (!in_transaction_ || !transaction_conn_) {
        return DatabaseError(DatabaseErrorType::TransactionFailed, "No transaction in progress");
      }
      ...
      if (error) {
        ...
        return DatabaseError(DatabaseErrorType::TransactionFailed, "Failed to commit transaction",
                             error);
      }
    ...
    DatabaseResult<void> rollback_transaction() {
      ...
      if (error) {
        ...
        return DatabaseError(DatabaseErrorType::TransactionFailed, "Failed to rollback transaction",
                             error);
      }
    ...
    DatabaseResult<void> with_transaction(TransactionFunc func) {
      ...
      try {
        ...
        if (result) {
          ...
          BOOST_LOG_TRIVIAL(warning)
              << "Transaction function failed: " << result.error().full_message();
          return result;
        }
    ...
    CacheStats get_cache_stats() const {
      ...
      stats.cache_memory_estimate
          = stats.cached_statements * (sizeof(PreparedStatement) + 100);  // Rough estimate
      return stats;
    }
