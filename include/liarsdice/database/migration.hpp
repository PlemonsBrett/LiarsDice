...
namespace liarsdice::database {
  ...
  public:
    ...
    Migration(int version, const std::string& description, const std::string& up_sql,
              const std::string& down_sql = "")
        : version_(version, description), up_sql_(up_sql), down_sql_(down_sql) {
      calculate_checksum();
    }

    Migration(int version, const std::string& description, MigrationFunc up_func,
              MigrationFunc down_func = nullptr)
        : version_(version, description), up_function_(up_func), down_function_(down_func) {
      calculate_checksum();
    }
     ...
     */
    bool is_reversible() const { return !down_sql_.empty() || down_function_ != nullptr; }
