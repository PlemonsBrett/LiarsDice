...
public:
    ...
    [[nodiscard]] T mean() const {
        ...
        auto integrand = [this](T theta) {
            return theta * posterior_pdf(theta);
        };
        
        return boost::math::quadrature::gauss_kronrod<T, 15>::integrate(
            integrand, lower, upper, 10, 1e-9);
    }
    ...
    [[nodiscard]] T mode() const {
        ...
        auto neg_log_posterior = [this](T theta) {
            return -log_posterior_pdf(theta);
        };
        ...
        auto result = boost::math::tools::brent_find_minima(
            neg_log_posterior, lower, upper, bits, max_iter);
    ...
    [[nodiscard]] T variance() const {
        ...
        auto integrand = [this](T theta) {
            return theta * theta * posterior_pdf(theta);
        };
        
        T second_moment = boost::math::quadrature::gauss_kronrod<T, 15>::integrate(
            integrand, lower, upper, 10, 1e-9);
    ...
    [[nodiscard]] std::pair<T, T> highest_density_interval(T confidence = 0.95) const {
        ...
        auto result = boost::math::tools::brent_find_minima(
            interval_width, lower_support, upper_support - (upper_support - lower_support) * confidence, 
            bits, max_iter);
     ...
     */
    [[nodiscard]] T marginal_likelihood() const {
        return std::exp(log_marginal_likelihood_);
    }
    ...
    [[nodiscard]] T information_gain() const {
        ...
        return boost::math::quadrature::gauss_kronrod<T, 15>::integrate(
            kl_integrand, lower, upper, 10, 1e-9);
    }
...
private:
    ...
    void update_conjugate(const vector_type& sufficient_stats) {
        ...
        if (likelihood_family == "bernoulli" || likelihood_family == "binomial") {
            ...
            if (beta_prior) {
                ...
                conjugate_posterior_ = std::make_unique<BetaPrior<T>>(
            ...
            auto normal_likelihood = dynamic_cast<NormalKnownVarianceLikelihood<T>*>(
                likelihood_.get());
            ...
            if (normal_prior && normal_likelihood) {
                ...
                conjugate_posterior_ = std::make_unique<NormalPrior<T>>(
                    posterior_mean, std::sqrt(posterior_var));
            }
            ...
            if (gamma_prior) {
                ...
                conjugate_posterior_ = std::make_unique<GammaPrior<T>>(
                    shape + sum_x, rate + n);
            }
        }
    }
     ...
     */
    [[nodiscard]] T posterior_pdf(T theta) const {
        return std::exp(log_posterior_pdf(theta));
    }
    ...
    [[nodiscard]] T posterior_cdf(T theta) const {
        ...
        auto integrand = [this](T t) {
            return posterior_pdf(t);
        };
        
        return boost::math::quadrature::gauss_kronrod<T, 15>::integrate(
            integrand, lower, theta, 10, 1e-9);
    }
