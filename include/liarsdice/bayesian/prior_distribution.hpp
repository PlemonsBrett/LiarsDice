...
namespace liarsdice::bayesian {
  ...
  public:
     ...
     */
    [[nodiscard]] virtual std::vector<T> sample(std::size_t n, boost::random::mt19937& gen) const {
   ...
   */
  template <typename T = double> class BetaPrior : public PriorDistribution<T> {
  public:
     ...
     */
    BetaPrior(T alpha, T beta) : dist_(alpha, beta), alpha_(alpha), beta_(beta) {
      if (alpha <= 0 || beta <= 0) {
    ...
    T mean() const override { return boost::math::mean(dist_); }

    T variance() const override { return boost::math::variance(dist_); }
    ...
    std::pair<T, T> support() const override { return {0, 1}; }
    ...
    bool is_conjugate_to(const std::string& likelihood_family) const override {
      return likelihood_family == "bernoulli" || likelihood_family == "binomial";
    }
   ...
   */
  template <typename T = double> class NormalPrior : public PriorDistribution<T> {
  public:
    using distribution_type = boost::math::normal_distribution<T>;

    NormalPrior(T mean, T std_dev) : dist_(mean, std_dev), mean_(mean), std_dev_(std_dev) {
      if (std_dev <= 0) {
        throw std::invalid_argument("Standard deviation must be positive");
      }
    }

    T pdf(T x) const override { return boost::math::pdf(dist_, x); }
    ...
    T cdf(T x) const override { return boost::math::cdf(dist_, x); }
   ...
   */
  template <typename T = double> class GammaPrior : public PriorDistribution<T> {
  public:
    using distribution_type = boost::math::gamma_distribution<T>;

    GammaPrior(T shape, T rate) : dist_(shape, 1 / rate), shape_(shape), rate_(rate) {
      if (shape <= 0 || rate <= 0) {
    ...
    T log_pdf(T x) const override {
      ...
      return shape_ * std::log(rate_) - std::lgamma(shape_) + (shape_ - 1) * std::log(x)
             - rate_ * x;
    }
    ...
    std::pair<T, T> support() const override { return {0, std::numeric_limits<T>::infinity()}; }
    ...
    bool is_conjugate_to(const std::string& likelihood_family) const override {
      return likelihood_family == "poisson" || likelihood_family == "exponential";
    }
   ...
   */
  template <typename T = double> class UniformPrior : public PriorDistribution<T> {
  public:
    using distribution_type = boost::math::uniform_distribution<T>;

    UniformPrior(T lower, T upper) : dist_(lower, upper), lower_(lower), upper_(upper) {
      if (lower >= upper) {
   ...
   */
  template <typename T = double>
  std::unique_ptr<PriorDistribution<T>> create_prior(const std::string& distribution_name,
