...
public:
     ...
     */
    [[nodiscard]] virtual vector_type sufficient_statistics(
        const vector_type& observations) const = 0;
 ...
 */
template<typename T = double>
class BernoulliLikelihood : public LikelihoodFunction<T> {
public:
    ...
    std::optional<std::string> conjugate_prior_family() const override {
        return "beta";
    }
};
 ...
 */
template<typename T = double>
class BinomialLikelihood : public LikelihoodFunction<T> {
public:
    using typename LikelihoodFunction<T>::vector_type;
 ...
 */
template<typename T = double>
class NormalKnownVarianceLikelihood : public LikelihoodFunction<T> {
public:
    using typename LikelihoodFunction<T>::vector_type;
 ...
 */
template<typename T = double>
class PoissonLikelihood : public LikelihoodFunction<T> {
public:
    using typename LikelihoodFunction<T>::vector_type;
 ...
 */
template<typename T = double>
class CustomLikelihood : public LikelihoodFunction<T> {
public:
    using typename LikelihoodFunction<T>::vector_type;
    using typename LikelihoodFunction<T>::function_type;
    
    CustomLikelihood(function_type likelihood_fn, 
 ...
 */
template<typename T = double>
std::unique_ptr<LikelihoodFunction<T>> create_likelihood(
    ...
    if (family_name == "bernoulli") {
        ...
        return std::make_unique<BinomialLikelihood<T>>(
            static_cast<std::size_t>(parameters[0]));
