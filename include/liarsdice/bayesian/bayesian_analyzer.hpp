...
namespace liarsdice::bayesian {
  ...
  public:
     ...
     */
    void set_prior(prior_ptr prior) { prior_ = std::move(prior); }

    /**
     * @brief Set the likelihood function for analysis
     * @param likelihood Shared pointer to likelihood function
     */
    void set_likelihood(likelihood_ptr likelihood) { likelihood_ = std::move(likelihood); }
     ...
     */
    static T bayes_factor(BayesianAnalyzer& model1, BayesianAnalyzer& model2,
    ...
    DiagnosticInfo get_diagnostics() const {
      ...
      return {posterior_->effective_sample_size(), posterior_->information_gain(),
              posterior_->convergence_metric(), posterior_->num_observations()};
    }
   ...
   */
  template <typename T = double> std::shared_ptr<BayesianAnalyzer<T>> create_conjugate_analyzer(
