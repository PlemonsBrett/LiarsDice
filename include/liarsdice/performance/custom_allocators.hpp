...
namespace liarsdice::performance {
   ...
   */
  template <typename T, std::size_t BlockSize = 32> class FastPoolAllocator {
  public:
    using value_type = T;
    using pointer = T*;
    ...
    using pool_type
        = boost::singleton_pool<pool_tag, sizeof(T), boost::default_user_allocator_new_delete,
                                boost::details::pool::null_mutex, BlockSize>;
    ...
    static void release_memory() { pool_type::release_memory(); }

    static void purge_memory() { pool_type::purge_memory(); }
  };
   ...
   */
  template <typename T, std::size_t Alignment = 32> using SimdAllocator
   ...
   */
  template <typename T> class GameObjectPool {
  public:
    explicit GameObjectPool(std::size_t initial_size = 16) : pool_(initial_size) {}
  ...
  namespace allocators {
    ...
    template <typename T> using AIDecisionAllocator = boost::pool_allocator<T>;
   ...
   */
  template <typename Allocator> class TrackedAllocator : public Allocator {
  public:
