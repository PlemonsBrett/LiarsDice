...
namespace liarsdice::validation {
  ...
  template <typename T> class IValidator {
  public:
  ...
  public:
    StringLengthValidator(size_t min_length, size_t max_length, const std::string& error_msg = "")
        : min_length_(min_length), max_length_(max_length), error_message_(error_msg) {
      if (error_message_.empty()) {
        error_message_ = "String length must be between " + std::to_string(min_length) + " and "
  ...
  template <typename T> class FunctionValidator : public IValidator<T> {
  public:
  ...
  // Composite validator
  template <typename T> class CompositeValidator : public IValidator<T> {
  public:
    ...
    std::string get_error_message() const override { return "Composite validation failed"; }
  ...
  namespace factory {
    ...
    inline auto dice_value_validator() {
      return std::make_unique<RangeValidator<unsigned int>>(1, 6,
    ...
    inline auto player_count_validator() {
      return std::make_unique<RangeValidator<unsigned int>>(2, 8,
    ...
    inline auto player_name_validator() {
      ...
      composite->add_validator(
          std::make_unique<StringLengthValidator>(1, 20, "Player name must be 1-20 characters"));
    ...
    inline auto file_path_validator() {
      return std::make_unique<RegexValidator>(R"(^[^<>:"|?*]+$)", "Invalid file path characters");
    }
  ...
  template <typename T> void validate_and_throw(const T& value, const IValidator<T>& validator) {
  ...
  template <typename T> bool is_valid(const T& value, const IValidator<T>& validator) {
