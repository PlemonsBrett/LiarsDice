...
public:
     ...
     */
    [[nodiscard]] T get(size_type row, size_type col) const {
     ...
     */
    [[nodiscard]] std::vector<std::tuple<size_type, size_type, T>> 
    find_top_n(size_type n) const {
        ...
        std::partial_sort(elements.begin(), 
                         elements.begin() + std::min(n, elements.size()),
                         elements.end(),
