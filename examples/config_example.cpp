...
int main() {
  ...
  std::cout << "Changed max players to: "
            << get_config<uint32_t>(ConfigPath{"game.rules.max_players"}).value_or(0) << "\n";

  set_config<std::string>(ConfigPath{"ui.theme"}, "dark");
  std::cout << "Changed theme to: "
            << get_config<std::string>(ConfigPath{"ui.theme"}).value_or("unknown") << "\n";
