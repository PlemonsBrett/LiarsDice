...
BOOST_DATA_TEST_CASE(PlayerIdValidation, bdata::make({1u, 2u, 5u, 10u, 100u, 999u, 1000u}),
                     player_id) {
...
BOOST_AUTO_TEST_CASE(PlayerPerformanceBenchmarks) {
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    (void)values;
    (void)count;  // Suppress unused warnings
  }
  ...
  BOOST_TEST_MESSAGE("Player operations performance: " << duration.count() << " microseconds for "
...
BOOST_AUTO_TEST_CASE(PlayerEliminationSimulation) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(dice_counts.begin(), dice_counts.end(), expected.begin(),
                                expected.end());
