...
template <typename Func> double measure_time(Func func, std::size_t iterations = 1) {
...
BOOST_AUTO_TEST_CASE(SimdVsScalarDotProduct) {
  ...
  std::cout << std::setw(10) << "Size" << std::setw(20) << "Scalar (ns)" << std::setw(20)
            << "SIMD (ns)" << std::setw(15) << "Speedup"
            << "\n";
  ...
  for (auto size : sizes) {
    ...
    std::cout << std::setw(10) << size << std::setw(20) << std::fixed << std::setprecision(2)
              << scalar_time << std::setw(20) << std::fixed << std::setprecision(2) << simd_time
...
BOOST_AUTO_TEST_CASE(SimdStatisticsPerformance) {
  ...
  std::cout << "Standard calculation: " << std::fixed << std::setprecision(2) << std_time
            << " ns\n";
  std::cout << "SIMD calculation: " << std::fixed << std::setprecision(2) << simd_time << " ns\n";
...
BOOST_AUTO_TEST_CASE(PoolAllocatorVsStandard) {
  ...
  std::cout << "Standard allocator: " << std::fixed << std::setprecision(2) << std_time << " ns\n";
  std::cout << "Pool allocator: " << std::fixed << std::setprecision(2) << pool_time << " ns\n";
  std::cout << "Speedup: " << std::fixed << std::setprecision(2) << speedup << "x\n";
...
BOOST_AUTO_TEST_CASE(MemoryArenaPerformance) {
  ...
  std::cout << "Standard allocation: " << std::fixed << std::setprecision(2) << std_time << " ns\n";
  std::cout << "Arena allocation: " << std::fixed << std::setprecision(2) << arena_time << " ns\n";
  std::cout << "Speedup: " << std::fixed << std::setprecision(2) << speedup << "x\n";
...
BOOST_AUTO_TEST_CASE(SimdAllocatorAlignment) {
  ...
  std::cout << "Aligned access: " << std::fixed << std::setprecision(2) << aligned_time << " ns\n";
  std::cout << "Unaligned access: " << std::fixed << std::setprecision(2) << unaligned_time
            << " ns\n";
...
BOOST_AUTO_TEST_CASE(CircularBufferPerformance) {
  ...
  auto stats_time = measure_time(
      [&]() {
        auto [mean, std_dev, min, max] = buffer.calculate_statistics([](double x) { return x; });
        [[maybe_unused]] volatile double m = mean;
      },
      1000);
  ...
  std::cout << "Window extraction (100 elements): " << std::fixed << std::setprecision(2)
            << window_time << " ns\n";
  std::cout << "Statistics calculation: " << std::fixed << std::setprecision(2) << stats_time
            << " ns\n";
...
BOOST_AUTO_TEST_CASE(StatisticalAccumulatorPerformance) {
  ...
  std::cout << "Accumulator (single-pass): " << std::fixed << std::setprecision(2) << acc_time
            << " ns\n";
  std::cout << "Traditional (multi-pass): " << std::fixed << std::setprecision(2) << trad_time
            << " ns\n";
  std::cout << "Efficiency gain: " << std::fixed << std::setprecision(2) << speedup << "x\n";
...
BOOST_AUTO_TEST_CASE(PoolAllocatorStatistics) {
  ...
  std::cout << "Pool size after purge: " << FastPoolAllocator<int>::get_allocated_count() << "\n";
}
