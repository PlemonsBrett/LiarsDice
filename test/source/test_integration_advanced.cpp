...
BOOST_DATA_TEST_CASE(GameScalabilityWithVariousPlayerCounts,
    bdata::make({2, 3, 4, 5, 6, 7, 8}),
    num_players) {
...
BOOST_AUTO_TEST_CASE(GamePerformanceUnderLoad) {
    ...
    BOOST_TEST_MESSAGE("Performance test: " << duration.count() 
                      << " ms for " << num_games << " games");
}
...
BOOST_AUTO_TEST_CASE(RealWorldGameplayScenario) {
    ...
    BOOST_TEST_MESSAGE("Realistic game: " << total_rounds << " rounds, " 
                      << dice_lost << " dice lost");
}
