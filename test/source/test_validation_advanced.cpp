...
BOOST_AUTO_TEST_CASE(ValidationPerformanceBenchmarks) {
    ...
    BOOST_TEST_MESSAGE("Validation performance: " << duration.count() 
                      << " microseconds for " << iterations << " validations");
}
...
BOOST_AUTO_TEST_CASE(ValidationUnicodeAndInternationalization) {
    ...
    std::vector<std::string> arabic_indic = {
        "٠", "١", "٢", "٣", "٤", "٥", "٦", "٧", "٨", "٩"
    };
...
BOOST_AUTO_TEST_CASE(ValidationStatelessBehavior) {
    ...
    std::vector<std::string> test_inputs = {
        "0", "1", "2", "5", "8", "9", "10", "abc", "", "quit"
    };
    
    for (const auto& input : test_inputs) {
        BOOST_CHECK_EQUAL(
            validator1.validate_player_count(input),
            validator2.validate_player_count(input)
        );
        
        BOOST_CHECK_EQUAL(
            validator1.validate_dice_count(input),
            validator2.validate_dice_count(input)
        );
        
        BOOST_CHECK_EQUAL(
            validator1.validate_face_value(input),
            validator2.validate_face_value(input)
        );
        
        BOOST_CHECK_EQUAL(
            validator1.validate_command(input),
            validator2.validate_command(input)
        );
    }
}
