...
BOOST_DATA_TEST_CASE(DiceValidationProperties,
                     bdata::make({0u, 1u, 2u, 3u, 4u, 5u, 6u, 7u, 8u, 9u, 10u, 100u}), test_value) {
  Dice dice;
...
BOOST_AUTO_TEST_CASE(DicePerformanceCharacteristics) {
  ...
  BOOST_TEST_MESSAGE("Roll performance: " << duration.count() << " microseconds for " << iterations
                                          << " operations");
}
...
BOOST_AUTO_TEST_CASE(DiceRandomnessDistribution) {
  ...
  BOOST_TEST_MESSAGE("Frequency distribution: [" << frequency[0] << ", " << frequency[1] << ", "
...
BOOST_AUTO_TEST_CASE(DiceSequenceBehavior) {
  ...
  std::transform(dice_collection.begin(), dice_collection.end(), std::back_inserter(values),
