...
public:
  ...
  void cleanup_logging() { logging::core::get()->remove_all_sinks(); }

  std::string get_log_output() { return test_stream->str(); }
  ...
  std::filesystem::path get_test_log_file() { return test_dir / "test.log"; }
...
BOOST_AUTO_TEST_CASE(LoggingProvidesComprehensiveGameDiagnostics) {
  ...
  BOOST_CHECK(output.find("info") != std::string::npos || output.find("INFO") != std::string::npos);
}
...
BOOST_AUTO_TEST_CASE(LoggingFileOutputPerformance) {
  ...
  // Add file sink
  logging::add_file_log(keywords::file_name = log_file.string(),
  ...
  BOOST_TEST_MESSAGE("Logged " << num_messages << " messages in " << duration.count()
                               << " microseconds");
}
