...
BOOST_AUTO_TEST_CASE(GameFlow) {
    ...
    game.events().on_player_turn.connect([&](const Player& player) {
        player_turn_called = true;
    });
...
BOOST_AUTO_TEST_CASE(CallLiarScenario) {
    ...
    // Connect to events
    game.events().on_liar_called.connect([&](const Player&) {
        liar_called = true;
    });
    
    game.events().on_round_end.connect([&](unsigned int) {
        round_ended = true;
    });
...
BOOST_AUTO_TEST_CASE(PlayerElimination) {
    ...
    game.events().on_player_eliminated.connect([&](const Player&) {
        player_eliminated = true;
    });
    
    game.events().on_game_winner.connect([&](const Player&) {
        game_ended = true;
    });
