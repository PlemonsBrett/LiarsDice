...
public:
  ...
  ~ConfigTestFixture() { std::filesystem::remove_all(test_dir); }
...
BOOST_DATA_TEST_CASE(ConfigurationValidatesParameterRanges,
  ...
  std::string config_content = R"(
        game {
            min_players )" + std::to_string(std::max(2, player_count - 1))
                               + R"(
            max_players )" + std::to_string(player_count)
                               + R"(
            dice_per_player )" + std::to_string(dice_count)
                               + R"(
        }
        ui {
...
BOOST_AUTO_TEST_CASE(ConfigurationPerformanceBenchmarks) {
  ...
  std::string complex_config = R"(
        ...
        ui {
            ...
            call_liar_prompt "Call liar? (y/n)" quit_message "Thanks for playing!"
}
        ai {
    ...
    auto config_path = fixture.create_test_config(complex_config);
        ...
        for (int i = 0; i < iterations; ++i) {
          ...
          (void)ui_config;
          (void)game_config;  // Suppress unused warnings
        }
        ...
        BOOST_TEST_MESSAGE("Configuration performance: " << duration.count() << " microseconds for "
