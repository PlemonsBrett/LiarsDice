...
BOOST_AUTO_TEST_CASE(BasicConnectionTest) {
    ...
    BOOST_CHECK_EQUAL(static_cast<int>(conn.get_state()), static_cast<int>(DatabaseConnection::State::Disconnected));
    ...
    BOOST_CHECK(!error);
    BOOST_CHECK(conn.is_open());
    BOOST_CHECK_EQUAL(static_cast<int>(conn.get_state()), static_cast<int>(DatabaseConnection::State::Connected));
    BOOST_CHECK(conn.get() != nullptr);
