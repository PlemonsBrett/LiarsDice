...
public:
  ...
  int get_total() const override { return service_->get_value() + 100; }
...
BOOST_AUTO_TEST_CASE(simple_binding_and_resolution) {
  auto injector = DI_MAKE_INJECTOR(DI_BIND(ITestService, TestServiceImpl));
...
BOOST_AUTO_TEST_CASE(singleton_binding) {
  auto injector = DI_MAKE_INJECTOR(DI_BIND_SINGLETON(ITestService, TestServiceImpl));
...
BOOST_AUTO_TEST_CASE(instance_binding) {
  ...
  auto injector = DI_MAKE_INJECTOR(DI_BIND_INSTANCE(ITestService, custom_service));
...
BOOST_AUTO_TEST_CASE(mock_binding_for_tests) {
  auto test_injector
      = testing::make_test_injector(DI_TEST_BIND(ITestService, MockTestService),
                                    DI_BIND(IDependentService, DependentServiceImpl));
