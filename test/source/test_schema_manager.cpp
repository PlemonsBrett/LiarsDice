...
struct SchemaManagerFixture {
    SchemaManagerFixture() {
        ...
        test_dir = fs::temp_directory_path() / ("liarsdice_test_schema_" + 
...
BOOST_AUTO_TEST_CASE(LoadFromDirectoryTest) {
    ...
    create_migration_file("V001__first.up.sql", 
        "CREATE TABLE first_table (id INTEGER PRIMARY KEY)");
    create_migration_file("V001__first.down.sql", 
        "DROP TABLE first_table");
    
    create_migration_file("V002__second.up.sql", 
        "CREATE TABLE second_table (id INTEGER PRIMARY KEY)");
    create_migration_file("V002__second.down.sql", 
...
BOOST_AUTO_TEST_CASE(BaselineTest) {
    ...
    schema_mgr->add_migration(std::make_unique<Migration>(
        6, "After baseline",
        "CREATE TABLE after_baseline (id INTEGER PRIMARY KEY)"
    ));
...
BOOST_AUTO_TEST_CASE(ValidationTest) {
    ...
    schema_mgr->add_migration(std::make_unique<Migration>(
        1, "First", "CREATE TABLE t1 (id INTEGER)"
    ));
...
BOOST_AUTO_TEST_CASE(TransactionRollbackOnFailureTest) {
    ...
    schema_mgr->add_migration(std::make_unique<Migration>(
        1, "Create table", "CREATE TABLE test (id INTEGER PRIMARY KEY)"
    ));
    
    schema_mgr->add_migration(std::make_unique<Migration>(
        2, "Invalid SQL", "CREATE TABLE INVALID SYNTAX HERE"
    ));
...
BOOST_AUTO_TEST_CASE(FunctionalMigrationTest) {
    ...
    schema_mgr->add_migration(std::make_unique<Migration>(
        1, "Functional migration", up_func, down_func
    ));
