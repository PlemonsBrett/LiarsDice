...
namespace liarsdice::ai {
  ...
  EasyAIStrategy::EasyAIStrategy(unsigned int id, const EasyConfig& config)
      : AIPlayer(id, "Easy AI " + std::to_string(id),
        ...
        config_(config) {
    BOOST_LOG_TRIVIAL(info) << "Created EasyAIStrategy " << id << " (risk=" << config.risk_tolerance
                            << ", bluff=" << config.bluff_frequency << ")";
  }

  EasyAIStrategy::EasyAIStrategy(unsigned int id) : EasyAIStrategy(id, EasyConfig{}) {}
  ...
  bool EasyAIStrategy::decide_call_liar(const core::Guess& last_guess) {
    ...
    BOOST_LOG_TRIVIAL(debug) << get_name() << " calculates probability " << probability
                             << " vs threshold " << adjusted_threshold;

    return probability < adjusted_threshold;
  }

  double EasyAIStrategy::calculate_simple_probability(const core::Guess& guess,
  ...
  double EasyAIStrategy::apply_statistical_adjustment(double base_probability,
