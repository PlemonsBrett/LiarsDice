...
namespace liarsdice::ai {
  ...
  core::Guess AIPlayer::make_guess(const std::optional<core::Guess>& last_guess) {
    ...
    core::Guess guess
        = should_bluff ? generate_bluff_guess(last_guess) : generate_safe_guess(last_guess);

    BOOST_LOG_TRIVIAL(debug) << get_name() << " guesses: " << guess.dice_count << " x "
                             << guess.face_value;
  ...
  double AIPlayer::calculate_probability(const core::Guess& guess, size_t total_dice) const {
    ...
    if (our_matching_dice > 0) {
      ...
      size_t needed
          = guess.dice_count > our_matching_dice ? guess.dice_count - our_matching_dice : 0;
  ...
  EasyAI::EasyAI(unsigned int id)
      : AIPlayer(id, "Easy AI " + std::to_string(id), Strategy{0.2, 0.1, 0.8, 500}) {}

  MediumAI::MediumAI(unsigned int id)
      : AIPlayer(id, "Medium AI " + std::to_string(id), Strategy{0.5, 0.25, 0.7, 1000}) {}

  HardAI::HardAI(unsigned int id)
      : AIPlayer(id, "Hard AI " + std::to_string(id), Strategy{0.8, 0.4, 0.6, 1500}) {}
  ...
  double HardAI::calculate_adjusted_probability(const core::Guess& guess, size_t total_dice) const {
    ...
    if (it != opponent_models_.end() && it->second.guess_history.size() >= 3) {
      ...
      if (face_it != it->second.face_preferences.end()) {
        double preference_rate
            = static_cast<double>(face_it->second) / it->second.guess_history.size();
