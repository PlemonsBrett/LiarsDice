...
namespace liarsdice::database {
  ...
  DatabaseResult<int> SchemaManager::load_migrations_from_directory(
  ...
  DatabaseResult<std::vector<SchemaVersion>> SchemaManager::get_applied_migrations() {
    ...
    auto result = db_.execute_prepared(stmt.value(), [&versions](const PreparedStatement& row) {
      SchemaVersion ver;
  ...
  DatabaseResult<bool> SchemaManager::is_version_applied(int version) {
    ...
    bool exists = false;
    db_.execute_prepared(stmt.value(), [&exists](const PreparedStatement&) { exists = true; });
  ...
  DatabaseResult<void> SchemaManager::migrate_to(int target_version) {
    ...
    return db_.with_transaction(
        [this, current_version, target_version](DatabaseManager& mgr) -> DatabaseResult<void> {
            ...
            if (version > current_version && version <= target_version) {
              ...
              if (!is_applied.value()) {
                ...
                BOOST_LOG_TRIVIAL(info)
                    << "Migration " << version << " applied in " << duration.count() << "ms";
              }
            }
          }
  ...
  DatabaseResult<void> SchemaManager::validate_migrations() {
    ...
    for (const auto& applied : applied_result.value()) {
      ...
      auto it = std::find_if(migrations_.begin(), migrations_.end(), [&applied](const auto& m) {
      ...
      if (it != migrations_.end()) {
        if ((*it)->get_version().checksum != applied.checksum) {
          return DatabaseError(
              DatabaseErrorType::InvalidParameter,
  ...
  DatabaseResult<void> SchemaManager::clean_migration_history() {
    ...
    return db_.with_transaction([this,
                                 &applied_result](DatabaseManager& mgr) -> DatabaseResult<void> {
      for (const auto& applied : applied_result.value()) {
        auto it = std::find_if(migrations_.begin(), migrations_.end(), [&applied](const auto& m) {
          return m->get_version().version == applied.version;
        });
        ...
        if (it == migrations_.end()) {
          ...
          BOOST_LOG_TRIVIAL(warning) << "Removed orphaned migration record: " << applied.version;
        }
      }
