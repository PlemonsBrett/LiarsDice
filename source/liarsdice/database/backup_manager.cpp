...
namespace liarsdice::database {
  ...
  DatabaseResult<BackupInfo> BackupManager::create_backup(const std::string& backup_name) {
    try {
      ...
      std::string filename
          = backup_name.empty() ? generate_backup_filename("manual") : backup_name + ".db";
      ...
      if (file_size_mb > retention_policy_.max_backup_size_mb) {
        ...
        return DatabaseError(DatabaseErrorType::BackupFailed,
                             "Backup size (" + std::to_string(file_size_mb)
      ...
      if (retention_policy_.compress_backups) {
        ...
        if (compress_result) {
          ...
          BOOST_LOG_TRIVIAL(warning)
              << "Failed to compress backup: " << compress_result.error().full_message();
        }
      }
  ...
  DatabaseResult<BackupInfo> BackupManager::create_scheduled_backup(
  ...
  DatabaseResult<void> BackupManager::restore_from_backup(
    ...
    try {
      ...
      bool is_compressed
          = boost::algorithm::ends_with(backup_file.string(), retention_policy_.compression_suffix);
  ...
  DatabaseResult<boost::filesystem::path> BackupManager::compress_backup(
      const boost::filesystem::path& backup_file) {
    if (!boost::filesystem::exists(backup_file)) {
      return DatabaseError(DatabaseErrorType::InvalidParameter,
                           "Backup file not found: " + backup_file.string());
    ...
    try {
      boost::filesystem::path compressed_file
          = backup_file.string() + retention_policy_.compression_suffix;
      ...
      BOOST_LOG_TRIVIAL(info) << "Compressed backup: " << backup_file << " -> " << compressed_file
  ...
  DatabaseResult<boost::filesystem::path> BackupManager::decompress_backup(
    ...
    try {
      ...
      if (boost::algorithm::ends_with(decompressed_name, retention_policy_.compression_suffix)) {
        decompressed_name = decompressed_name.substr(
            0, decompressed_name.length() - retention_policy_.compression_suffix.length());
      } else {
  ...
  std::string BackupManager::generate_backup_filename(const std::string& backup_type) {
    ...
    if (backup_type == "daily")
      pattern = retention_policy_.daily_pattern;
    else if (backup_type == "weekly")
      pattern = retention_policy_.weekly_pattern;
    else if (backup_type == "monthly")
      pattern = retention_policy_.monthly_pattern;
    else if (backup_type == "yearly")
      pattern = retention_policy_.yearly_pattern;
    else {
      // Manual backup
  ...
  BackupInfo BackupManager::create_backup_info(const boost::filesystem::path& file_path,
    ...
    info.is_compressed
        = boost::algorithm::ends_with(file_path.string(), retention_policy_.compression_suffix);
  ...
  std::vector<BackupInfo> BackupManager::scan_backup_directory() {
    ...
    try {
      for (boost::filesystem::directory_iterator it(backup_dir_);
        ...
        if (boost::filesystem::is_regular_file(it->path()) && is_backup_file(it->path())) {
    ...
    return boost::algorithm::starts_with(filename, "backup_")
           && (boost::algorithm::ends_with(filename, ".db")
               || boost::algorithm::ends_with(filename,
                                              ".db" + retention_policy_.compression_suffix));
  }
  ...
  std::string BackupManager::format_backup_filename(
      const std::string& pattern, const std::chrono::system_clock::time_point& time_point) {
  ...
  DatabaseResult<void> BackupManager::schedule_automatic_backups(bool enable) {
    ...
    BOOST_LOG_TRIVIAL(info) << "Automatic backup scheduling " << (enable ? "enabled" : "disabled");
    return DatabaseResult<void>();
  }
