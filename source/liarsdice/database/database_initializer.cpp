...
DatabaseResult<void> DatabaseInitializer::initialize_from_json(const boost::filesystem::path& config_file) {
...
DatabaseResult<void> DatabaseInitializer::initialize_from_xml(const boost::filesystem::path& config_file) {
    if (!boost::filesystem::exists(config_file)) {
        return DatabaseError(DatabaseErrorType::InvalidParameter, 
                           "Configuration file not found: " + config_file.string());
    }
...
DatabaseResult<void> DatabaseInitializer::initialize_from_config(const DatabaseInitConfig& config) {
    ...
    if (config.run_migrations && !config.migration_directory.empty()) {
        ...
        auto migrations_result = schema_mgr_.load_migrations_from_directory(config.migration_directory);
        if (!migrations_result) {
            BOOST_LOG_TRIVIAL(warning) << "Failed to load migrations: " 
                                      << migrations_result.error().full_message();
        } else {
    ...
    if (config.seed_data && !config.seed_data_file.empty()) {
        ...
        if (!seed_result) {
            BOOST_LOG_TRIVIAL(warning) << "Failed to load seed data: " 
                                      << seed_result.error().full_message();
        } else {
            auto apply_result = apply_seed_data(seed_result.value());
            if (!apply_result) {
                BOOST_LOG_TRIVIAL(warning) << "Failed to apply seed data: " 
                                          << apply_result.error().full_message();
            } else {
                BOOST_LOG_TRIVIAL(info) << "Applied " << seed_result.value().size() << " seed data entries";
            }
        }
    }
...
DatabaseResult<std::vector<SeedDataEntry>> DatabaseInitializer::load_seed_data_json(const boost::filesystem::path& data_file) {
...
DatabaseResult<void> DatabaseInitializer::apply_seed_data(const std::vector<SeedDataEntry>& seed_data) {
    try {
...
DatabaseResult<void> DatabaseInitializer::create_config_template(const boost::filesystem::path& output_file, 
...
DatabaseResult<DatabaseInitConfig> DatabaseInitializer::parse_json_config(const boost::property_tree::ptree& pt) {
...
DatabaseResult<DatabaseInitConfig> DatabaseInitializer::parse_xml_config(const boost::property_tree::ptree& pt) {
    // Similar to JSON parsing but for XML structure
    return parse_json_config(pt); // For simplicity, use same logic
}

DatabaseResult<void> DatabaseInitializer::configure_database_settings(const DatabaseInitConfig& config) {
    try {
...
DatabaseResult<void> DatabaseInitializer::execute_pragma_settings(const DatabaseInitConfig& config) {
...
DatabaseResult<void> DatabaseInitializer::seed_achievements_data() {
    ...
    for (const auto& [code, name, desc, points] : achievements) {
        std::ostringstream sql;
        sql << "INSERT OR IGNORE INTO achievements (code, name, description, points, category) VALUES ("
...
DatabaseResult<void> DatabaseInitializer::validate_initialization_requirements(const DatabaseInitConfig& config) {
...
DatabaseResult<void> DatabaseInitializer::create_required_tables(const std::vector<std::string>& tables) {
