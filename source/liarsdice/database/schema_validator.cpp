...
namespace liarsdice::database {
  ...
  void SchemaValidator::initialize_constraints() {
    ...
    table_naming.validator = [this](const std::string& name) { return validate_table_name(name); };
    ...
    column_naming.validator
        = [this](const std::string& name) { return validate_column_name(name); };
    constraints_.push_back(column_naming);

    // Primary key constraints for main tables
    constraints_.push_back(
        SchemaConstraint("players", "id", ConstraintType::PrimaryKey, "pk_players"));
    constraints_.push_back(
        SchemaConstraint("game_history", "id", ConstraintType::PrimaryKey, "pk_game_history"));
    constraints_.push_back(
        SchemaConstraint("achievements", "id", ConstraintType::PrimaryKey, "pk_achievements"));
    ...
    constraints_.push_back(SchemaConstraint("game_participants", "player_id",
                                            ConstraintType::ForeignKey, "fk_participant_player",
                                            "REFERENCES players(id)"));
    constraints_.push_back(SchemaConstraint("game_participants", "game_id",
                                            ConstraintType::ForeignKey, "fk_participant_game",
                                            "REFERENCES game_history(id)"));

    // Unique constraints
    constraints_.push_back(
        SchemaConstraint("players", "username", ConstraintType::Unique, "uk_players_username"));
    constraints_.push_back(
        SchemaConstraint("achievements", "code", ConstraintType::Unique, "uk_achievements_code"));
  ...
  DatabaseResult<ValidationReport> SchemaValidator::validate_schema() {
    ...
    try {
      ...
      BOOST_LOG_TRIVIAL(info) << "Schema validation completed: "
                              ...
                              << report.errors.size() << " errors, " << report.warnings.size()
                              << " warnings)";
      ...
      for (const auto& constraint : constraints_) {
        ...
        switch (constraint.type) {
          ...
          case ConstraintType::NotNull:
            if (schema.count("sql")
                && schema["sql"].find(constraint.column_name + " ") != std::string::npos) {
    ...
    try {
      ...
      for (const auto& table : tables) {
        ...
        if (!validate_table_name(trimmed)) {
          report.add_error("Table name '" + table
                           + "' violates naming convention (should be lowercase_with_underscores)");
        }
        ...
        for (const auto& fk : fk_list) {
          ...
          if (parts.size() >= 3 && boost::algorithm::to_upper_copy(parts[0]) == "REFERENCES") {
            ...
            if (std::find(tables.begin(), tables.end(), ref_table) == tables.end()) {
              report.add_error("Foreign key in table '" + table
                               + "' references non-existent table '" + ref_table + "'");
            }
          }
        }
      }
    ...
    try {
      ...
      for (const auto& [index_name, table_name, sql] : indexes) {
        if (!validate_index_naming(index_name, table_name)) {
          report.add_warning("Index '" + index_name
                             + "' doesn't follow naming convention (idx_table_column)");
        }
      }
  ...
  std::map<std::string, std::string> SchemaValidator::get_table_schema(
  ...
  bool SchemaValidator::validate_table_name(const std::string& name) {
    ...
    return std::all_of(name.begin(), name.end(),
                       [](char c) { return std::islower(c) || std::isdigit(c) || c == '_'; })
  ...
  bool SchemaValidator::check_constraint_exists(const std::string& table,
  ...
  bool SchemaValidator::validate_index_naming(const std::string& index_name,
                                              const std::string& table_name) {
