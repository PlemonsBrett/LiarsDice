...
template<>
std::shared_ptr<BayesianAnalyzer<double>> create_conjugate_analyzer<double>(
    ...
    if (likelihood_family == "bernoulli" || likelihood_family == "binomial") {
        ...
        if (likelihood_family == "bernoulli") {
            ...
            analyzer->set_likelihood(std::make_shared<BinomialLikelihood<double>>(
        ...
        double prior_mean = hyperparameters.count("prior_mean") ? 
            hyperparameters.at("prior_mean") : 0.0;
        double prior_std = hyperparameters.count("prior_std") ? 
            hyperparameters.at("prior_std") : 1.0;
        ...
        analyzer->set_likelihood(std::make_shared<NormalKnownVarianceLikelihood<double>>(
            likelihood_var));
    }
