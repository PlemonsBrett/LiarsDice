...
namespace liarsdice::app {
  ...
  Application::Application() { setup_signal_handlers(); }
  ...
  int Application::run(int argc, char* argv[]) {
    try {
      ...
      BOOST_LOG_TRIVIAL(info) << "=== Liar's Dice v1.0.0.0 (Boost Edition) ===";
  ...
  void Application::init_logging() {
    boost::log::core::get()->set_filter(
        ...
        >= (config_.verbose ? boost::log::trivial::debug : boost::log::trivial::info));
  ...
  po::options_description Application::setup_program_options() {
    ...
    desc.add_options()("help,h", "Show help message")("verbose,v", "Enable verbose output")(
        "demo", "Run in demo mode")("min-players",
                                    po::value<unsigned int>(&config_.min_players)->default_value(2),
  ...
  void Application::show_welcome() { print_with_flush(WELCOME_MSG); }
  ...
  void Application::setup_game(int total_players, int ai_players) {
    BOOST_LOG_TRIVIAL(info) << "Setting up game: " << total_players << " total players, "
    ...
    game_->events().on_round_start.connect([this](unsigned int round) { on_round_start(round); });

    game_->events().on_player_turn.connect(
        [this](const core::Player& player) { on_player_turn(player); });

    game_->events().on_guess_made.connect(
        [this](const core::Player& player, const core::Guess& guess) {
          on_guess_made(player, guess);
        });

    game_->events().on_liar_called.connect(
        [this](const core::Player& player) { on_liar_called(player); });

    game_->events().on_round_result.connect(
        [this](const core::Player& caller, const core::Player& loser, int points_lost) {
          on_round_result(caller, loser, points_lost);
        });

    game_->events().on_player_eliminated.connect(
        [this](const core::Player& player) { on_player_eliminated(player); });

    game_->events().on_game_winner.connect(
        [this](const core::Player& winner) { on_game_winner(winner); });
  ...
  void Application::handle_human_turn(core::Player& player) {
    ...
    if (last_guess.has_value()) {
      ...
      std::cout << "Previous guess: " << last_guess->dice_count << " dice showing "
                << last_guess->face_value << std::endl;
    }
  ...
  core::Guess Application::get_player_guess(const std::optional<core::Guess>& last_guess) {
    ...
    while (!should_exit.load()) {
      ...
      if (!game_->is_valid_guess(guess)) {
        print_with_flush(INVALID_MSG);
        if (last_guess.has_value()) {
          std::cout << "You must either increase the dice count or keep the same count with a "
                       "higher face value."
                    << std::endl;
          std::cout << "Previous guess was: " << last_guess->dice_count << " dice showing "
                    << last_guess->face_value << std::endl;
        }
        continue;
      }
  ...
  void Application::show_game_state() {
    ...
    auto last_guess = game_->get_last_guess();
    if (last_guess.has_value()) {
      std::cout << "Last guess: " << last_guess->dice_count << " dice showing "
                << last_guess->face_value << std::endl;
    }
    std::cout << "==================\n" << std::endl;
  }
  ...
  void Application::on_guess_made(const core::Player& player, const core::Guess& guess) {
    std::cout << player.get_name() << " guesses: " << guess.dice_count << " dice showing "
              << guess.face_value << std::endl;
  }
  ...
  void Application::on_round_result(const core::Player& caller, const core::Player& loser,
    ...
    std::cout << loser.get_name() << " now has " << loser.get_points() << " points remaining."
              << std::endl;
  }
