...
namespace liarsdice::core {
  ...
  void Game::add_player(std::shared_ptr<Player> player) {
    ...
    if (existing != players_.end()) {
      BOOST_LOG_TRIVIAL(warning) << "Player with ID " << player->get_id() << " already exists";
      return;
    }
    ...
    BOOST_LOG_TRIVIAL(info) << "Added player " << player->get_id() << " (" << player->get_name()
                            << ") with " << player->get_dice_count() << " dice";
  ...
  void Game::start_game() {
    ...
    if (players_.size() < config_.min_players) {
      BOOST_LOG_TRIVIAL(warning) << "Cannot start game - not enough players (" << players_.size()
                                 << "/" << config_.min_players << ")";
      return;
    }
  ...
  void Game::process_guess(const Guess& guess) {
    ...
    BOOST_LOG_TRIVIAL(info) << "Player " << guess.player_id << " guesses: " << guess.dice_count
                            << " dice showing " << guess.face_value;
  ...
  void Game::process_call_liar() {
    ...
    if (actual_count >= last_guess_->dice_count) {
      ...
      BOOST_LOG_TRIVIAL(info) << "Liar call failed - caller loses " << points_lost
                              << " points for false accusation";
    } else {
      ...
      BOOST_LOG_TRIVIAL(info) << "Liar call succeeded - guesser loses " << points_lost
                              << " point for lying";
    }
  ...
  void Game::end_game() {
    ...
    if (!active_players_.empty()) {
      ...
      BOOST_LOG_TRIVIAL(info) << "Game over! Winner: " << winner->get_name() << " (Player "
                              << winner->get_id() << ")";
  ...
  void Game::eliminate_player(std::shared_ptr<Player> player) {
    BOOST_LOG_TRIVIAL(info) << "Player " << player->get_id() << " (" << player->get_name()
                            << ") eliminated";
  ...
  void Game::log_game_state() const {
    ...
    for (const auto& player : active_players_) {
      BOOST_LOG_TRIVIAL(debug) << "  Player " << player->get_id() << " (" << player->get_name()
                               << "): " << player->get_dice_count() << " dice";
    }

    if (last_guess_.has_value()) {
      BOOST_LOG_TRIVIAL(debug) << "Last guess: " << last_guess_->dice_count << " dice showing "
                               << last_guess_->face_value;
    }
  ...
  void Game::capture_game_state() {
    ...
    for (const auto& player : players_) {
      ...
      state_storage_.store_player_state(static_cast<GameStateStorage::PlayerId>(player->get_id()),
                                        state);

      // Update active players set
      if (!player->is_eliminated()) {
        state_storage_.add_active_player(static_cast<GameStateStorage::PlayerId>(player->get_id()));
      }
    }
    ...
    BOOST_LOG_TRIVIAL(trace) << "Game state captured - " << state_storage_.size() << " players, "
