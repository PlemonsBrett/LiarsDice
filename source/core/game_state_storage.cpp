...
void CompactGameState::set_dice_value(std::size_t index, std::uint8_t value) {
    if (index >= MAX_DICE || value < 1 || value > 6) {
        BOOST_LOG_TRIVIAL(warning) << "Invalid dice index or value: " 
...
std::uint32_t CompactGameState::serialize() const {
    ...
    result |= static_cast<std::uint32_t>(*reinterpret_cast<const std::uint8_t*>(&player_state)) << 16;
    result |= static_cast<std::uint32_t>(*reinterpret_cast<const std::uint8_t*>(&last_action)) << 24;
    return result;
}
...
GameHistory::GameHistory(std::size_t max_entries) 
    : history_(max_entries) {
