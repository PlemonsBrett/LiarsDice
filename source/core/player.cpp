...
namespace liarsdice::core {
  ...
  Player::Player(unsigned int id, const std::string& name)
      : id_(id),
        name_(name.empty() ? "Player " + std::to_string(id) : name),
    ...
    BOOST_LOG_TRIVIAL(debug) << "Created player: " << name_ << " (ID: " << id_ << ") with "
                             << points_ << " points";
  }
  ...
  Guess HumanPlayer::make_guess(const std::optional<Guess>& last_guess) {
    ...
    if (last_guess) {
      std::cout << "Previous guess: " << last_guess->dice_count << " x " << last_guess->face_value
                << "\n";
    }
  ...
  bool HumanPlayer::decide_call_liar(const Guess& last_guess) {
    ...
    std::cout << "Current guess: " << last_guess.dice_count << " x " << last_guess.face_value
              << "\n";
